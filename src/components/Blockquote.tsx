...
function BlockquoteWithImage({
  ...
  return (
    ...
    >
      <blockquote className="col-span-2 text-xl/7 text-neutral-600 dark:text-neutral-400 sm:col-span-7 sm:col-start-6 sm:row-start-2">
        {typeof children === 'string' ? <p>{children}</p> : children}
      </blockquote>
      <div className="col-start-1 row-start-2 overflow-hidden rounded-xl bg-neutral-100 dark:bg-neutral-900 sm:col-span-5 sm:row-span-full sm:rounded-3xl">
        <Image
          alt=""
          {...image}
      ...
      <figcaption className="text-sm text-neutral-950 dark:text-white sm:col-span-7 sm:row-start-3 sm:text-base">
  ...
  return (
    <Border position="left" className={clsx('pl-8', className)}>
      <figure className="text-sm">
        <blockquote className="text-neutral-600 dark:text-neutral-400 [&>*]:relative [&>:first-child]:before:absolute [&>:first-child]:before:right-full [&>:first-child]:before:content-['“'] [&>:last-child]:after:content-['”']">
          {typeof children === 'string' ? <p>{children}</p> : children}
        </blockquote>
        <figcaption className="mt-6 font-semibold text-neutral-950 dark:text-white">
